...
<action>Format the roster for documentation with each agent on its own line: "- [Display Name] ([Title]): [Role]"</action>
<template-output>agent_roster</template-output>
</step>

<step n="2" goal="Initialize Party Mode">
  <action>Announce party mode activation with enthusiasm using the roster from Step 1</action>
  <action>List all participating agents with their merged information inside the announcement</action>
  <format>
  ...
  <template-output>party_activation_message</template-output>
  <ask response="discussion_topic">What topic should the team dive into first?</ask>
  <template-output>discussion_topic</template-output>
</step>

<step n="3" goal="Orchestrate Multi-Agent Discussion" repeat="until-exit">
  <action>For each user message or topic:</action>
  <action>Use {{discussion_topic}} when beginning the first round if no new prompt is provided</action>
  ...
  <substep n="3d" goal="Format and Present Responses">
    ...
    <action>Append the current user message and the agents' responses to a running conversation log</action>
    <action>Ensure the saved conversation_log content always includes the full history so far</action>
    <template-output>conversation_log</template-output>
...
<step n="4" goal="Exit Party Mode">
  ...
  <template-output>farewell_messages</template-output>
