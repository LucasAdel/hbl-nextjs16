  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <nav className="mb-6 flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
        <Link href="/library" className="hover:text-tiffany-600 transition-colors flex items-center gap-1">
          <ChevronLeft className="h-4 w-4" />
          Back to Library
        </Link>
        <span>/</span>
        <Link
          href={`/library/categories?category=${encodeURIComponent(article.category)}`}
          ...
          <header className="space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {article.tags.map((tag) => (
                <Link
                  key={tag}
                  href={`/library/search?tag=${encodeURIComponent(tag)}`}
