...
import type { ArticleCategory, LibraryArticle } from "@/lib/codex/types";
...
const categoryConfig: Record<ArticleCategory, {
  ...
  "Corporate Law": {
    ...
    darkColor: "dark:bg-indigo-900/30",
  ...
  "Business": {
    ...
    darkColor: "dark:bg-violet-900/30",
...
function CategoriesContent() {
  ...
  return (
    <div className="min-h-screen">
      {/* Hero Section */}
      <section className="bg-gradient-to-br from-tiffany-lighter via-white to-tiffany-light dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-16 lg:py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-4xl mx-auto text-center">
            {/* Badge */}
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/80 dark:bg-slate-800/80 backdrop-blur-sm shadow-sm border border-tiffany-200 dark:border-slate-700 mb-6 animate-fade-in-up">
            ...
            <h1 className="font-serif text-4xl lg:text-6xl text-gray-900 dark:text-white mb-6 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
              Browse by Category
            </h1>

            <p className="text-xl text-gray-600 dark:text-gray-300 mb-12 max-w-3xl mx-auto animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 text-center animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
              <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-2xl p-4 border border-gray-200/50 dark:border-slate-700/50">
              ...
              <div className="bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm rounded-2xl p-4 border border-gray-200/50 dark:border-slate-700/50">
                ...
                <div className="text-sm text-gray-600 dark:text-gray-400">Australian Focus</div>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center gap-2 bg-gray-100 dark:bg-slate-800 rounded-lg p-1">
              <button
                ...
                className={`p-2 rounded-md transition-colors ${
                  viewMode === "grid"
                    ? "bg-white dark:bg-slate-700 text-tiffany shadow-sm"
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${isSelected
                        ? "bg-tiffany-50 dark:bg-tiffany-900/20 border-tiffany-300 dark:border-tiffany-700"
                        : "bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700 hover:border-tiffany-200 dark:hover:border-tiffany-700"
                      }
                    `}
                    style={{ animationDelay: `${index * 0.05}s` }}
                  >
                    <div className={`${config.lightColor} ${config.darkColor} rounded-lg p-3 shrink-0`}>
                      <Icon className={`h-5 w-5 ${config.textColor} ${config.darkTextColor}`} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        ...
                        <span className={`
                          ...
                          ${isSelected
                            ? "bg-tiffany-200 dark:bg-tiffany-800 text-tiffany-700 dark:text-tiffany-300"
                    ...
                    <ChevronRight className={`h-5 w-5 shrink-0 transition-transform ${isSelected ? "rotate-90 text-tiffany-600 dark:text-tiffany-400" : "text-gray-400 dark:text-gray-500"}`} />
                  </button>
                );
              }
                  ...
                  >
                    <div className="p-6">
                      ...
                      <div className="flex items-center mb-6">
                        <div className={`w-12 h-12 ${config.lightColor} ${config.darkColor} rounded-xl flex items-center justify-center mr-4`}>
                          <Icon className={`w-6 h-6 ${config.textColor} ${config.darkTextColor}`} />
                        </div>
                        <div>
                          <h3 className="font-serif text-xl text-gray-900 dark:text-white">
                            {category}
                          </h3>
                          <p className="text-sm text-gray-500 dark:text-gray-400">{count} article{count !== 1 ? 's' : ''}</p>
                        </div>
                      </div>
                      ...
                      {categoryArticles.length > 0 && (
                        <div className="space-y-3 mb-6">
                          <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-200">Recent Articles:</h4>
                      ...
                      <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-slate-700">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${config.lightColor} ${config.darkColor} ${config.textColor} ${config.darkTextColor}`}>
                          {count} Articles
                        </span>
                        <span className="inline-flex items-center gap-1 text-tiffany-600 dark:text-tiffany-400 font-medium">
                          Explore Category
              ...
              {otherCategories.map((category, index) => {
                ...
                return (
                  <button
                    ...
                    onClick={() => setSelectedCategory(isSelected ? null : category)}
                    className={`
                      block p-6 bg-white dark:bg-slate-800 rounded-xl border transition-all duration-300 hover:shadow-lg text-left
                      ${isSelected
                        ? "border-tiffany-400 dark:border-tiffany-600"
                        : "border-gray-200 dark:border-slate-700 hover:border-tiffany-200 dark:hover:border-tiffany-700"
                      }
                    `}
                    style={{ animationDelay: `${index * 0.05}s` }}
                  >
                    <div className="flex items-center justify-between mb-4">
                      <div className="flex items-center gap-3">
                        <div className={`${config.lightColor} ${config.darkColor} p-2 rounded-lg`}>
                          <Icon className={`h-4 w-4 ${config.textColor} ${config.darkTextColor}`} />
                        </div>
                        <h3 className="font-semibold text-gray-900 dark:text-white">
                          {category}
                        </h3>
                      </div>
                      <span className="bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 px-2 py-1 rounded-full text-xs font-medium">
                        {count}
                      </span>
                    </div>
                    ...
                    <div className="inline-flex items-center gap-1 text-tiffany-600 dark:text-tiffany-400 font-medium text-sm">
                      View Articles
                      <ArrowRight className="w-4 h-4" />
                    </div>
                  </button>
            ...
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                {(() => {
                  ...
                  return (
                    <div className={`${config?.lightColor || "bg-gray-100"} ${config?.darkColor || "dark:bg-gray-900/30"} p-3 rounded-xl`}>
                      <Icon className={`h-6 w-6 ${config?.textColor || "text-gray-600"} ${config?.darkTextColor || "dark:text-gray-400"}`} />
                    </div>
                  );
                })()}
                <div>
        ...
        <section className="py-16 bg-white dark:bg-slate-900">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center py-12 bg-gray-50 dark:bg-slate-800/50 rounded-2xl">
              ...
              <button
                onClick={() => setSelectedCategory(null)}
                className="text-tiffany-600 dark:text-tiffany-400 font-medium hover:text-tiffany-700 dark:hover:text-tiffany-300"
              >
...
function ArticleCard({
  ...
  return (
    <Link
      href={getLibraryPath(`/codex/${article.slug}`)}
      className="group bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 hover:shadow-lg hover:border-tiffany-200 dark:hover:border-tiffany-700 transition-all duration-300"
      style={{ animationDelay: `${index * 0.1}s` }}
    >
      {/* Badges */}
      ...
      <h4 className="font-semibold text-lg text-gray-900 dark:text-white mb-3 group-hover:text-tiffany-600 dark:group-hover:text-tiffany-400 transition-colors">
        {article.title}
      </h4>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400 pt-4 border-t border-gray-100 dark:border-slate-700">
        <span className="flex items-center gap-1">
          <Clock className="h-3.5 w-3.5" />
          {article.readingTime} min read
        </span>
        <span className="flex items-center gap-1 text-tiffany-600 dark:text-tiffany-400 font-medium">
          Read More
