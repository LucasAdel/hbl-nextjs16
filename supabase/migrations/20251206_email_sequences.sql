...
CREATE TABLE IF NOT EXISTS email_sequence_enrollments (
  ...
  email TEXT NOT NULL,
  sequence_type TEXT NOT NULL CHECK (sequence_type IN ('welcome_series', 'post_consultation', 'post_purchase', 'booking_reminder', 're_engagement', 'cart_abandonment')),
...
-- Add name column to newsletter_subscribers if not exists
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'newsletter_subscribers' AND column_name = 'name'
  ) THEN
