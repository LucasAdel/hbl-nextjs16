...
DROP POLICY IF EXISTS "Service role full access to abandoned_carts" ON abandoned_carts;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE table_name = 'abandoned_carts' AND column_name = 'abandoned_at'
  ) THEN
    ALTER TABLE abandoned_carts ADD COLUMN abandoned_at TIMESTAMPTZ DEFAULT NOW();
  END IF;
END $$;
